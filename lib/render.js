...
function subtitleFor(text, index, plan) {
  ...
  if (plan?.subtitles?.[index]) return shortText(vietnameseText(plan.subtitles[index]), 56);
  return shortText(cleaned, 56);
}
...
function sceneTitleFor(line, index, plan) {
  const planned = plan.titles?.[index];
  if (planned && !isTechnicalTitle(planned)) return planned;
...
function buildScenes(job) {
  ...
  return script.map((line, index) => ({
    title: sceneTitleFor(line, index, plan),
    body: line,
...
function syntheticVisualHtml(index, scene, labels, icons) {
  const title = shortText(scene.title, 34);
  const text = shortText(scene.subtext || scene.body, 84);
  ...
  if (index % 5 === 2) {
    return `
      <div class="synthetic flow-visual">
        ...
        ${[title, labels[0], labels[1], labels[2], "Kết quả"].map((node, i) => `<em class="big-node node-${i}">${escapeHtml(shortText(node, 22))}</em>`).join("")}
      </div>`;
  }
  if (index % 5 === 3) {
    return `
...
function publicSourceLabel(job, fallback = "Nguồn đã chọn") {
  const name = vietnameseText(job.sourceContext?.name || job.topic || "", "");
  if (name) {
    const isRepo = job.sourceContext?.repo || (job.links || []).some((link) => /github\.com/i.test(String(link || "")));
    return shortText(isRepo ? `${name} trên GitHub` : name, 44);
  }
  const raw = job.sourceContext?.fullName || fallback;
  return shortText(String(raw || fallback).replace(/[-_]+/g, " "), 44);
}
...
function publicSceneTitle(scene, index) {
  ...
  return shortText(vietnameseText(title), 46);
}
...
function comboVisualCopy(comboId, index, scene, job, media, fallbackSource = "Nguồn đã chọn") {
  ...
  const body = shortText(scene.body, 118);
  const subtext = shortText(scene.subtext || scene.body, 76);
...
function mediaFrameHtml(mediaNode, scene, copy, extraClass = "") {
  return `
    <div class="media-frame cue media-float ${extraClass}">
      ...
      <div class="object-subtitle cue">${escapeHtml(scene.subtext)}</div>
...
function flowMiniHtml(nodes) {
  return `
    <div class="flow-mini cue">
      ...
      ${nodes.map((node, i) => `<span class="flow-node cue n${i}">${escapeHtml(shortText(node, 24))}</span>`).join("")}
    </div>`;
}
...
function visualHtml(scene, index, job, combo) {
  ...
  if (combo.id === "privacy-trust-review") {
    ...
    if (variant === 1) {
      ...
      layout = `
        <div class="trust-ledger cue">
          ${["Nên thử khi", "Cần kiểm tra", "Không nên nói quá"].map((label, i) => `<span><b>${escapeHtml(label)}</b><small>${escapeHtml(shortText([copy.body, copy.proof, copy.subtext][i], 74))}</small></span>`).join("")}
        </div>
    ...
    if (variant === 1) {
      layout = `
        ...
        <div class="compare-timeline cue">
          ${["Trước", "Khác biệt", "Kết quả"].map((label, i) => `<span><b>${escapeHtml(label)}</b><small>${escapeHtml(shortText([copy.body, scene.subtext, copy.proof][i], 58))}</small></span>`).join("")}
        </div>
      ...
      layout = `
        ...
        <div class="compare-grid cue">
          <div class="compare-panel before"><span>Trước</span><p>${escapeHtml(shortText(copy.body, 74))}</p></div>
          <div class="compare-panel after"><span>Sau</span><p>${escapeHtml(shortText(scene.subtext || copy.proof, 74))}</p></div>
        </div>
    ...
    const stepCards = `<div class="step-stack cue compact-steps">
      ${steps.map((step, i) => `<div class="step-card"><b>0${i + 1}</b><span>${escapeHtml(step)}</span><p>${escapeHtml(shortText(i === 0 ? copy.proof : i === 1 ? copy.body : copy.subtext, 66))}</p></div>`).join("")}
    </div>`;
    layout = variant === 1
    ...
    layout = `
      ...
      <div class="metric-grid cue">
        ${metrics.map((metric, i) => `<div class="metric-pill"><b>${escapeHtml(metric)}</b><span style="--w:${78 + i * 7}%"></span><small>${escapeHtml(shortText([copy.proof, copy.body, copy.subtext][i], 54))}</small></div>`).join("")}
      </div>
...
function renderHtml(job, catalogs) {
  ...
  const timelineJs = scenes.map((scene, index) => {
    ...
    tl.to("#scene${index} .big-node", {autoAlpha:1, scale:1, duration:0.26, stagger:0.05, ease:"back.out(1.8)"}, ${at(0.22, 0.55)});
    ...
    tl.to("#scene${index} .flow-node", {autoAlpha:1, scale:1, duration:0.26, stagger:0.05, ease:"back.out(1.7)"}, ${at(0.23, 0.58)});
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .kinetic-word { position:absolute; font-size:72px; line-height:.92; font-weight:900; text-transform:uppercase; color:#fff; text-shadow:0 0 20px ${palette.primary}, 0 0 60px ${palette.accent}; opacity:0; visibility:hidden; transform:translate(-50%,-50%); }
    ...
    .flow-node { position:absolute; min-width:116px; max-width:150px; min-height:48px; padding:10px 12px; border-radius:16px; display:grid; place-items:center; text-align:center; background:linear-gradient(135deg, ${palette.primary}, ${palette.accent}); color:#fff; font-size:16px; font-weight:900; box-shadow:0 16px 34px rgba(0,0,0,.24); transform:translate(-50%,-50%) scale(.4); opacity:0; }
    ...
    .dynamic-3 .media-frame { transform:translateZ(40px); }
...
async function processJob(job, state, onLog) {
  ...
  if (job.options.voice && job.options.voice !== "none") {
    ...
    if (job.voicePath && fs.existsSync(job.voicePath)) {
      ...
      if (voiceDuration > 0) {
        const syncedDuration = Math.max(6, Math.ceil((voiceDuration + 1.2) * 10) / 10);

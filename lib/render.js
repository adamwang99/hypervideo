...
function scriptFromJob(job) {
  ...
  const bodyTemplates = {
    ...
    "app-demo-flow": [
      `Bạn có nguồn đầu vào, nhưng lại mất thời gian nối từng bước để biến nó thành kết quả dùng được ngay.`,
      `${facts.name} gom các bước đó vào cùng một luồng: lấy nguồn, rút ý chính, rồi tạo đầu ra ngay trong một chỗ.`,
    ...
    "tutorial-steps": [
      `Nếu đang cần bắt đầu nhanh với ${facts.name}, điều quan trọng nhất là biết đầu vào nào cần chuẩn bị trước.`,
      `Quy trình gọn nhất là: chọn đầu vào, chạy luồng chính, rồi kiểm tra kết quả trước khi dùng thật.`,
...
function subtitleFor(text, index, plan) {
  ...
  if (split.body && countWords(split.body) >= 6) return conciseText(split.body, 8, 52);
  const fragments = cleaned.split(/(?<=[.!?])\s+|[,;:]\s+/u).map((item) => item.trim()).filter(Boolean);
  const first = fragments[0] || cleaned || source;
  const candidate = countWords(first) >= 6 ? first : (cleaned || source);
  return conciseText(candidate || fragments[1] || source, 8, 52);
}
...
const COMBO_SCENE_TITLES = {
  "repo-launch-dynamic": ["Nhìn nhanh", "Điểm khác", "Bằng chứng", "Cách thử", "Cần biết", "Xem tiếp"],
  "evidence-explainer": ["Điểm nổi bật", "Bằng chứng", "Tín hiệu tốt", "Điều cần xem", "Kết lại", "Thử tiếp"],
  "infographic-cta": ["Điểm chính", "Con số đáng chú ý", "Ý chốt", "Cách hiểu nhanh", "Điều nên nhớ", "Thử ngay"],
  "app-demo-flow": ["Nhìn nhanh", "Đầu vào", "Cách chạy", "Đầu ra", "Điều đáng giá", "Thử ngay"],
  "privacy-trust-review": ["Giữ trên máy", "Điểm lợi", "Điều cần biết", "Rủi ro", "Nên thử nhỏ", "Kết lại"],
  "quick-compare": ["Trước đó", "Khác ở đâu", "Kết quả thấy rõ", "Điều cần cân nhắc", "Khi nên chọn", "Thử nhanh"],
  "tutorial-steps": ["Chuẩn bị", "Làm thế nào", "Kiểm tra lại", "Kết quả", "Lưu ý", "Làm tiếp"],
  "creator-story": ["Vấn đề ban đầu", "Bước ngoặt", "Bằng chứng", "Điểm đáng giá", "Điều cần cân nhắc", "Gọi hành động"]
};
...
function displayBodyForScene(scene, index, job = null) {
  ...
  if (split.body) {
    ...
    if (countWords(candidate) >= 7) return conciseText(candidate, 26, 180);
  }
  ...
  if (headline && structuredParts.length > 1) {
    ...
    if (firstPartKey && (firstPartKey === titleKey || firstPartKey.startsWith(titleKey) || titleKey.startsWith(firstPartKey))) {
      return conciseText(structuredParts.slice(1).join(". "), 26, 180);
    }
  }
  ...
  return conciseText(finalText, 26, 180);
}

function sceneTitleFor(line, index, plan, job) {
  const planned = plan.titles?.[index];
  if (goodDisplayTitle(planned) && countWords(planned) <= 6 && !/[.!?]/.test(planned)) return planned;
...
function publicComboSceneTitle(job, index) {
  return COMBO_SCENE_TITLES[job?.options?.templateCombo || ""]?.[index]
    || [
      "Điểm đáng chú ý",
      "Lợi ích rõ nhất",
      "Bằng chứng kiểm tra",
...
function buildScenes(job) {
  ...
  return script.map((line, index) => ({
    ...
    label: publicChipLabel((COMBO_FLOW_LABELS[job?.options?.templateCombo || ""] || [])[index] || plan.labels?.[index] || `Cảnh ${index + 1}`, index),
...
async function resolveJobScript(job, state, onLog) {
  ...
  try {
    ...
    const comboTitles = COMBO_SCENE_TITLES[job?.options?.templateCombo || ""] || [];
    const comboLabels = COMBO_FLOW_LABELS[job?.options?.templateCombo || ""] || [];
    ...
    job.contentPlan = {
      ...
      styleLabel: "LLM",
      titles: llm.scenes.map((scene, index) => comboTitles[index] || (isTechnicalTitle(scene.title) ? titleFromLine(scene.voice || scene.subtitle || "") : scene.title)),
      labels: displayLabels(llm.scenes.map((scene, index) => comboLabels[index] || scene.label || "SCENE"), llm.scenes.length),
...
function syntheticVisualHtml(index, scene, labels, icons, job = null) {
  ...
  const iconItems = (
    comboId === "app-demo-flow" ? ["Nguồn", "Luồng", "Kết quả", "Local"] :
    comboId === "tutorial-steps" ? ["Chuẩn bị", "Chạy", "Kiểm tra", "Xong"] :
    comboId === "quick-compare" ? ["Trước", "Sau", "Khác", "Thử"] :
    ["app-demo-flow", "tutorial-steps", "quick-compare"].includes(comboId) ? labels : icons
  ).slice(0, 6);
  const variantMap = {
...
function comboSemanticLabels(comboId, index, scene, job) {
  const base = COMBO_FLOW_LABELS[comboId] || COMBO_FLOW_LABELS["repo-launch-dynamic"];
...
function publicSceneTitle(scene, index, job = null) {
  const comboId = job?.options?.templateCombo || "";
  if (["app-demo-flow", "tutorial-steps", "quick-compare"].includes(comboId)) {
    return conciseText(publicComboSceneTitle(job || {}, index), 7, 46);
  }
  const derived = displayHeadlineFromLine(scene?.body || scene?.title || "", publicComboSceneTitle(job || {}, index));
  if (derived && !isTechnicalTitle(derived) && countWords(derived) <= 8) {
    return conciseText(derived, 7, 46);
  }
  const title = String(scene.title || "").trim();
  if (title && !isTechnicalTitle(title) && countWords(title) <= 8 && !/[.!?]/.test(title)) {
...
function comboVisualCopy(comboId, index, scene, job, media, fallbackSource = "Nguồn đã chọn") {
  ...
  const subtext = conciseText(countWords(rawSubtext) >= 6 ? rawSubtext : body || rawSubtext, 9, 54);
  ...
  return {
    source,
    title: conciseText(publicSceneTitle(scene, index, job), 7, 46),
...
function appDemoActionLines(copy, media) {
  ...
  return [
    ...
    ["Local", media ? "Có dẫn chứng trực tiếp từ repo." : conciseText(copy.source, 6, 40)]
  ];
}
...
function visualHtml(scene, index, job, combo) {
  ...
  const chipSource = combo.id === "app-demo-flow"
    ? ["Nguồn", "Luồng", "Kết quả", "Local"]
    : combo.id === "tutorial-steps"
      ? ["Chuẩn bị", "Chạy", "Kiểm tra", "Xong"]
      : combo.id === "quick-compare"
        ? ["Trước đó", "Bây giờ", "Khác gì", "Khi nên thử"]
...
function renderHtml(job, catalogs, options = {}) {
  ...
  const timelineJs = scenes.map((scene, index) => {
    ...
    ${index < scenes.length - 1 ? `tl.fromTo("#grid-pixelate-overlay .grid-cell", {scale:0}, {scale:1, duration:0.22, stagger:{amount:0.18, from:"center"}, ease:"power2.inOut"}, ${Math.max(scene.end - 0.42, scene.start + 0.5)});
    tl.fromTo("#grid-pixelate-overlay .grid-cell", {scale:1}, {scale:0, duration:0.22, stagger:{amount:0.18, from:"edges"}, ease:"power2.inOut"}, ${Math.max(scene.end - 0.18, scene.start + 0.7)});` : ""}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    #grid-pixelate-overlay .grid-cell { background:${palette.bg}; transform-origin:center; }
    ...
    #root.combo-app-demo-flow .source-media { max-height:none; object-fit:contain; }
    ...
    #root.combo-app-demo-flow .media-frame {
...
async function processJob(job, state, onLog) {
  ...
  if (job.options.voice && job.options.voice !== "none") {
    ...
    if (job.voicePath && fs.existsSync(job.voicePath)) {
      ...
      if (voiceDuration > 0) {
        const syncedDuration = Math.max(6, Math.ceil((voiceDuration + 0.8) * 10) / 10);
  ...
  if (job.voicePath) {
    ...
    const voiceDuration = await mediaDuration(job.voicePath);
    const finalDuration = voiceDuration > 0 ? Math.ceil((voiceDuration + 0.18) * 1000) / 1000 : 0;
    ...
    if (voiceDuration > videoDuration + 0.05) {
      muxArgs.push(
        "-vf", `tpad=stop_mode=clone:stop_duration=${(voiceDuration - videoDuration + 0.22).toFixed(3)}`,
        "-c:v", "libx264",
        "-pix_fmt", "yuv420p",
        "-preset", "veryfast",
        "-crf", "18"

...
function sceneTitleFor(line, index, plan, job) {
  ...
  const derived = titleFromLine(line, fallbackTitles[index] || "Điểm đáng chú ý");
  ...
  if (comboTitle && !isTechnicalTitle(comboTitle)) return comboTitle;
  return derived || fallbackTitles[index] || "Điểm đáng chú ý";
}
...
function syntheticVisualHtml(index, scene, labels, icons) {
  const title = conciseText(publicSceneTitle(scene, index), 6, 34);
...
function renderHtml(job, catalogs) {
  ...
  const sceneHtml = scenes.map((scene, index) => `
    <section id="scene${index}" class="scene scene-${index % 3}">
      <div class="copy">
        <div class="eyebrow title-pill cue">${escapeHtml(publicSceneTitle(scene, index))}</div>

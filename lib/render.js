...
function localGsapRuntimeScript() {
  return `<script>
(function(){
  if (window.gsap) return;
  const transforms = new WeakMap();
  const transformKeys = new Set(["x","y","scale","rotate","rotateX","rotateY"]);
  const tweenMeta = new Set(["duration","ease","stagger","delay","overwrite"]);
  ...
  function clean(props) {
    ...
    Object.keys(props || {}).forEach((key) => {
      if (!tweenMeta.has(key)) out[key] = props[key];
    });
    return out;
  }
  ...
  function interp(fromValue, toValue, progress) {
    if (progress >= 1) return toValue;
    if (progress <= 0 && fromValue !== undefined) return fromValue;
    if (typeof toValue === "number" || typeof fromValue === "number") {
      const from = num(fromValue, 0);
      const to = num(toValue, from);
      return from + (to - from) * progress;
    }
    const toText = String(toValue ?? "");
    const fromText = String(fromValue ?? "");
    const toMatch = toText.match(/^(-?\\d+(?:\\.\\d+)?)(.*)$/);
    const fromMatch = fromText.match(/^(-?\\d+(?:\\.\\d+)?)(.*)$/);
    if (toMatch && (!fromValue || fromMatch)) {
      const to = Number(toMatch[1]);
      const from = fromMatch ? Number(fromMatch[1]) : 0;
      return String(from + (to - from) * progress) + (toMatch[2] || "");
    }
    return progress >= 0.5 ? toValue : (fromValue ?? toValue);
  }
  function transformState(el) {
    if (!transforms.has(el)) transforms.set(el, {x:0,y:0,scale:1,rotate:0,rotateX:0,rotateY:0});
    return transforms.get(el);
  }
  function writeTransform(el) {
    const state = transformState(el);
    el.style.transform = "translate(" + state.x + "px, " + state.y + "px) rotate(" + state.rotate + "deg) rotateX(" + state.rotateX + "deg) rotateY(" + state.rotateY + "deg) scale(" + state.scale + ")";
  }
  function applyProp(el, key, value, progress, fromValue) {
    const next = interp(fromValue, value, progress);
    if (key === "autoAlpha") {
      const opacity = Number(next);
      el.style.opacity = String(Number.isFinite(opacity) ? opacity : (progress >= 1 ? 1 : 0));
      el.style.visibility = Number(el.style.opacity) <= 0.01 ? "hidden" : "visible";
      return;
    }
    if (key === "visibility") {
      if (progress >= 1) el.style.visibility = String(value);
      return;
    }
    if (transformKeys.has(key)) {
      const state = transformState(el);
      const fallback = key === "scale" ? 1 : 0;
      state[key] = num(next, fallback);
      writeTransform(el);
      return;
    }
    if (key === "opacity") {
      el.style.opacity = String(next);
      el.style.visibility = Number(next) <= 0.01 ? "hidden" : "visible";
      return;
    }
    if (key in el.style) {
      el.style[key] = String(next);
    }
  }
  function applyProps(el, props, progress, fromProps) {
    Object.keys(props || {}).forEach((key) => applyProp(el, key, props[key], progress, fromProps?.[key]));
  }
  function staggerStep(stagger, count) {
    if (!stagger || count <= 1) return 0;
    if (typeof stagger === "number") return stagger;
    if (typeof stagger.amount === "number") return stagger.amount / Math.max(1, count - 1);
    return 0;
  }
  class MiniTimeline {
    constructor() {
    ...
    set(target, props, at = 0) {
      this.items.push({type:"set", target, props: clean(props), start:Number(at) || 0, duration:0, stagger:props?.stagger});
      return this;
    }
    to(target, props, at = 0) {
      this.items.push({type:"to", target, props: clean(props), start:Number(at) || 0, duration:Number(props?.duration) || 0, stagger:props?.stagger});
      return this;
    }
    fromTo(target, fromProps, toProps, at = 0) {
      this.items.push({type:"fromTo", target, fromProps: clean(fromProps), props: clean(toProps), start:Number(at) || 0, duration:Number(toProps?.duration) || 0, stagger:toProps?.stagger});
      return this;
    }
    duration() {
    ...
    progress(value) {
      if (value === undefined) return this.duration() ? this._time / this.duration() : 0;
      return this.seek((Number(value) || 0) * this.duration());
    }
    pause() { return this; }
    play() { return this; }
    apply() {
      for (const item of this.items) {
        const elements = list(item.target);
        const step = staggerStep(item.stagger, elements.length);
        elements.forEach((el, index) => {
          const start = item.start + step * index;
          if (this._time < start) return;
          if (item.type === "set") {
            applyProps(el, item.props, 1, {});
            return;
          }
          const progress = item.duration ? clamp((this._time - start) / item.duration) : 1;
          if (item.type === "fromTo") {
            applyProps(el, item.fromProps, 1, {});
            applyProps(el, item.props, progress, item.fromProps);
          } else {
            applyProps(el, item.props, progress, {});
          }
        });
      }
    }
  ...
  window.gsap = {
    config() {},
    set(target, props) {
      list(target).forEach((el) => applyProps(el, clean(props), 1, {}));
    },
    timeline() {
      return new MiniTimeline();
    }
  };
})();
</script>`;
...
function stripDisplayTitlePrefix(value) {
  return cleanMarkdownLine(value)
    .replace(/^(?:câu chuyện\s*\/\s*giá trị|cau chuyen\s*\/\s*gia tri|cách dùng\s*\/\s*use\s*-?\s*case|cach dung\s*\/\s*use\s*-?\s*case|workflow\s*\/\s*use\s*-?\s*case)\s*/iu, "")
    .replace(/^(?:mở đầu|mo dau|giá trị|gia tri|bằng chứng|bang chung|nguồn|nguon|quy trình|quy trinh|phản biện|phan bien|cta|kết|ket|kết luận|ket luan|thực tế|thuc te|lợi ích|loi ich|điểm chốt|diem chot)\s*[–—:-]\s*/iu, "")
    .replace(/^(?:ý|y|cảnh|canh)\s*\d+\s*[–—:-]\s*/iu, "")
    .trim();
}
...
function finalizeDisplayPhrase(value) {
  const text = cleanMarkdownLine(value)
    .replace(/\s+/g, " ")
    .replace(/\b(?:và|nhưng|hoặc|rồi|để|vì|khi|nếu|với|trong|trên|cho|của|từ)$/iu, "")
    .replace(/[,;:–—-]+$/u, "")
    .trim();
  return text;
}
...
function normalizeVietnameseSpeechText(value) {
  ...
  text = text.replace(/(?<![\w/:-])(\d+)\.(\d{1,2})(?![\w/:-])/g, (_, whole, fraction) => {
    const head = numberToVietnamese(Number(whole)) || whole;
    const tail = String(fraction).split("").map((digit) => small[Number(digit)] || digit).join(" ");
    return `${head} phẩy ${tail}`.trim();
  });
  ...
  text = text
    ...
    .replace(/\bGPU\b/gi, "gi pi iu")
    .replace(/\bCUDA\b/gi, "cu đa")
    .replace(/\bMLX\b/gi, "em el ích");
  return text;
}
...
function run(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (!/unauthenticated requests to the HF Hub/i.test(chunk.toString())) {
...
function contentFacts(source, job) {
  const name = displayName(source.name || job.topic || "dự án này", source.name || job.topic || "dự án này");
...
function narrationReadyLine(line, index, lines) {
  const text = cleanMarkdownLine(line);
  if (!text) return "";
  const split = splitNarrationLead(text, "");
  if (split.body && countWords(split.headline) <= 10 && countWords(split.body) >= 5) {
    return cleanMarkdownLine(split.body);
  }
  if (isLikelyScriptHeading(text) && index < lines.length - 1 && !/[.!?…]$/.test(text)) {
    return `${text}.`;
  }
  return text;
}
...
function normalizeNarrativeSentence(value, fallback = "") {
  const raw = stripDisplayMetaPrefix(vietnameseText(value, fallback)).replace(/\s+/g, " ").trim();
  if (!raw) return cleanMarkdownLine(fallback || "");
  const first = raw.charAt(0).toUpperCase() + raw.slice(1);
  return /[.!?…]$/u.test(first) ? first : `${first}.`;
}

function conciseNarrative(value, fallback = "", max = 180) {
  return normalizeNarrativeSentence(shortText(String(value || "").trim(), max), fallback);
}

function firstNaturalFact(facts, ...indexes) {
  for (const index of indexes) {
    const value = facts.points?.[index];
    if (value && countWords(value) >= 6) return normalizeNarrativeSentence(value);
  }
  return "";
}

function sourceTopicsSentence(source, facts) {
  const topics = Array.isArray(source?.topics) ? source.topics.filter(Boolean).slice(0, 4) : [];
  if (!topics.length) return "";
  return `Các điểm như ${topics.join(", ")} cho thấy ${facts.name} không chỉ có ý tưởng, mà đã có hướng dùng khá rõ.`;
}

function sourceUseCaseSentence(source, facts) {
  const blockedTopics = new Set(["ai", "api", "sdk", "cli", "tts", "cuda", "mlx"]);
  const topics = Array.isArray(source?.topics)
    ? source.topics
        .filter(Boolean)
        .map((item) => String(item).trim())
        .filter((item) => item.length >= 4 && !/\d/.test(item) && !blockedTopics.has(item.toLowerCase()))
        .slice(0, 3)
    : [];
  if (topics.length >= 2) {
    return `Nhìn từ ngữ cảnh nguồn, ${facts.name} đang tập trung vào ${topics.join(", ")}, nên giá trị của nó nằm ở khả năng ghép các bước này thành một luồng dễ dùng hơn.`;
  }
  if (facts.promise) {
    return `${facts.name} phù hợp khi người dùng thật sự cần ${facts.promise}, chứ không chỉ muốn xem thêm một demo công nghệ.`;
  }
  return "";
}

function distinctFactPool(facts) {
  const pool = [];
  for (const item of facts.points || []) {
    if (!item || countWords(item) < 6) continue;
    const normalized = normalizeNarrativeSentence(item);
    if (!normalized) continue;
    if (pool.some((existing) => isNearDuplicateScriptLine(normalized, [existing]))) continue;
...
function fillToDurationLines(job, facts, lines) {
  ...
  for (const extra of extras) {
    ...
    output.splice(
      Math.max(3, output.length - 2),
      0,
      uniqueLine(
        output,
        extra,
        sourceUseCaseSentence(source, facts) || `${facts.name} chỉ nên được đánh giá theo đúng bằng chứng đang có trong nguồn hiện tại.`
      )
    );
  }
...
function scriptFromJob(job) {
  ...
  const factPool = distinctFactPool(facts);
  const primaryFact = factPool[0]
    || firstNaturalFact(facts, 0, 1, 2)
    || conciseNarrative(facts.description, `${facts.name} có một giá trị đủ rõ để thử ngay trên nhu cầu thực tế.`);
  const proofFact = factPool.find((item) => !isNearDuplicateScriptLine(item, [primaryFact]))
    || `${facts.name} có mô tả công khai đủ rõ để người xem tự kiểm tra, không cần tin mù quáng.`;
  const useCaseFact = factPool.find((item) => !isNearDuplicateScriptLine(item, [primaryFact, proofFact]))
    || sourceUseCaseSentence(source, facts)
    || `${facts.name} hợp với người đang cần bớt thao tác rời rạc và muốn có một luồng làm việc dễ kiểm soát hơn.`;
  const productFact = factPool.find((item) => !isNearDuplicateScriptLine(item, [primaryFact, proofFact, useCaseFact]))
    || conciseNarrative(facts.description, `${facts.name} gom nhiều bước về cùng một trải nghiệm dễ theo dõi hơn.`);
  const topicSentence = sourceTopicsSentence(source, facts);
  const lines = [];
  const openingVariants = {
    "repo-launch-dynamic": `${facts.name} không phải kiểu repo chỉ để xem cho biết. Nó đáng chú ý vì có thể biến ${facts.promise} thành một quy trình đủ rõ để thử ngay.`,
    "evidence-explainer": `Thay vì nghe lời giới thiệu, hãy nhìn vào ${facts.name} qua thứ quan trọng hơn: bằng chứng thật và cách nó giải quyết vấn đề.`,
    "infographic-cta": `${visibleMetric ? `${visibleMetric} là tín hiệu đầu tiên.` : "Có một tín hiệu đáng chú ý."} Nhưng điều đáng xem hơn là ${facts.name} biến tín hiệu đó thành lợi ích cụ thể thế nào.`,
    "app-demo-flow": `Nếu bạn muốn thấy sản phẩm làm được gì trong vài giây, ${facts.name} nên được kể như một luồng thao tác chứ không phải một danh sách tính năng.`,
    "privacy-trust-review": `Điểm mạnh của ${facts.name} không chỉ nằm ở tính năng. Nó nằm ở việc dữ liệu ở đâu, chạy ở đâu và người dùng còn giữ được quyền kiểm soát đến đâu.`,
    "quick-compare": `Trước đây bạn phải tự nối nhiều bước rời rạc. Với ${facts.name}, câu hỏi hay hơn là: nó rút ngắn được đoạn nào đủ rõ để đáng đổi cách làm cũ?`,
    "tutorial-steps": `Nếu mới nhìn vào ${facts.name}, cách hiểu nhanh nhất là xem nó nhận đầu vào gì, xử lý ra sao và trả lại kết quả ở đâu.`,
    "creator-story": `Câu chuyện của ${facts.name} bắt đầu từ một nhu cầu rất thật: muốn làm nhanh hơn, rõ hơn, nhưng không muốn quy trình vỡ vụn vì quá nhiều bước.`
  };
  lines.push(normalizeNarrativeSentence(openingVariants[comboId] || applyContentTemplate(writingProfile.opening || style[3], styleValues)));
  const bodyTemplates = {
    "infographic-cta": [
      primaryFact,
      productFact,
      proofFact,
      topicSentence || sourceUseCaseSentence(source, facts) || `Nếu nhìn ở góc thực dụng, ${facts.name} hợp với người cần chốt nhanh giá trị chính mà không phải đọc hết toàn bộ nguồn.`
    ],
    "app-demo-flow": [
      productFact,
      primaryFact,
    ...
    "evidence-explainer": [
      ...
      proofFact,
      productFact,
      topicSentence || `Nếu chưa có đủ số liệu, cách đúng là giữ giọng điệu tỉnh táo và chỉ diễn giải những gì nguồn hiện có thật sự cho phép.`
    ],
    "repo-launch-dynamic": [
      primaryFact,
      useCaseFact,
    ...
    "privacy-trust-review": [
      ...
      productFact,
      proofFact,
      `Nếu bạn ưu tiên quyền riêng tư, đây là kiểu công cụ nên thử nhỏ trước rồi mới quyết định có đưa vào việc chính hay không.`
    ],
    "quick-compare": [
      productFact,
      primaryFact,
      proofFact,
      `Khác biệt đáng giá không nằm ở số tính năng, mà ở chỗ người dùng có làm xong việc nhanh hơn và rõ hơn hay không.`
    ],
    "tutorial-steps": [
      productFact,
      primaryFact,
      useCaseFact,
      `Nếu muốn thử an toàn, hãy bắt đầu bằng một tác vụ nhỏ để xem đầu ra có đúng thứ bạn cần hay không.`
    ],
    "creator-story": [
      productFact,
      primaryFact,
      proofFact,
      `Vì vậy, người xem không chỉ thấy một tính năng riêng lẻ, mà thấy cả một cách làm việc liền mạch hơn.`
    ]
  };
  for (const templateLine of bodyTemplates[comboId] || [
    ...
    lines.push(uniqueLine(
      lines,
      normalizeNarrativeSentence(templateLine),
      `${facts.name} có một giá trị đủ rõ từ nguồn hiện có để người xem tự kiểm tra và thử trên nhu cầu thật.`
    ));
  }
  if (contentMode === "strict-source") {
  ...
  lines.push(uniqueLine(
    lines,
    normalizeNarrativeSentence(applyContentTemplate(writingProfile.counter || `${facts.name} không thay thế quyết định của bạn. Giá trị của nó nằm ở việc giảm bước lặp, giúp phát hiện vấn đề sớm và giữ quy trình dễ kiểm soát hơn.`, facts)),
  ...
  lines.push(normalizeNarrativeSentence(cta));
  ...
  job.contentPlan = {
    ...
    labels: lines.map((line, index) => {
      const semantic = sceneSemanticPhrases(
        {
          title: titleFromLine(line, titleFallbacks[index] || "Điểm đáng chú ý"),
          subtext: "",
          body: line
        },
        []
      )[0];
      return semantic || defaultLabels[index] || extraLabels[(index - defaultLabels.length) % extraLabels.length];
    })
  };
...
function subtitleFor(text, index, plan) {
  const source = plan?.subtitles?.[index] ? vietnameseText(plan.subtitles[index]) : vietnameseText(text);
  const cleaned = stripDisplayMetaPrefix(source);
  const split = splitNarrationLead(cleaned, "");
  const firstClause = (value) => value
    .split(/\s*[,;:–—]\s*/u)
    .map((item) => item.trim())
    .find((item) => countWords(item) >= 5) || "";
  if (split.body && countWords(split.body) >= 6) {
    const clause = firstClause(split.body);
    if (clause) return finalizeDisplayPhrase(conciseText(clause, 12, 84));
    return finalizeDisplayPhrase(conciseText(split.body, 12, 84));
  }
  const sentences = splitTextBySentences(cleaned);
  const firstSentence = sentences.find((item) => countWords(item) >= 6) || "";
  if (firstSentence) {
    const clause = firstClause(firstSentence);
    if (clause) return finalizeDisplayPhrase(conciseText(clause, 12, 84));
    return finalizeDisplayPhrase(conciseText(firstSentence, 12, 84));
  }
  const fragments = cleaned.split(/(?<=[.!?])\s+|[,;:]\s+/u).map((item) => item.trim()).filter(Boolean);
  const candidate = fragments.find((item) => countWords(item) >= 5) || cleaned || source;
  return finalizeDisplayPhrase(conciseText(candidate, 12, 84));
}
...
const COMBO_SCENE_TITLES = {
  "repo-launch-dynamic": ["Vì sao đáng thử", "Điểm khác biệt", "Bằng chứng rõ", "Thử như thế nào", "Cần cân nhắc", "Bắt đầu ngay"],
  "evidence-explainer": ["Điểm nổi bật", "Bằng chứng rõ", "Tín hiệu đáng tin", "Điều cần xem", "Kết luận nhanh", "Thử tiếp"],
  "infographic-cta": ["Điểm chính", "Con số đáng nhớ", "Ý chốt", "Cách hiểu nhanh", "Điều nên nhớ", "Thử ngay"],
  "app-demo-flow": ["Vì sao đáng thử", "Luồng local", "Cách chạy gọn", "Kết quả thấy được", "Điều cần để ý", "Bắt đầu nhanh"],
  "privacy-trust-review": ["Giữ trên máy", "Điểm lợi thật", "Điều cần biết", "Rủi ro còn lại", "Nên thử nhỏ", "Kết lại"],
  "quick-compare": ["Trước đây", "Khác ở đâu", "Kết quả thấy rõ", "Điều cần cân nhắc", "Khi nên chọn", "Thử nhanh"],
  "tutorial-steps": ["Chuẩn bị trước", "Làm từng bước", "Kiểm tra lại", "Kết quả nhận được", "Lưu ý khi dùng", "Làm tiếp"],
  "creator-story": ["Vấn đề ban đầu", "Bước ngoặt", "Bằng chứng thật", "Điểm đáng giá", "Điều cần cân nhắc", "Gọi hành động"]
};

const COMBO_FLOW_LABELS = {
  "repo-launch-dynamic": ["Điểm mạnh", "Nguồn", "Demo", "Thử ngay"],
  "evidence-explainer": ["Nguồn", "Dẫn chứng", "Giới hạn", "Quyết định"],
  "infographic-cta": ["Con số", "Lợi ích", "Tác động", "Hành động"],
  "app-demo-flow": ["Nguồn vào", "Xử lý", "Đầu ra", "Chạy local"],
  "privacy-trust-review": ["Local", "Dữ liệu", "Kiểm soát", "Rủi ro"],
  "quick-compare": ["Trước đó", "Bây giờ", "Khác gì", "Khi nên thử"],
  "tutorial-steps": ["Chuẩn bị", "Chạy thử", "Kiểm tra", "Hoàn tất"],
  "creator-story": ["Vấn đề", "Cú chuyển", "Kết quả", "Lời kết"]
};

function goodDisplayTitle(value) {
  const text = stripDisplayTitlePrefix(value);
  return text
    && !isTechnicalTitle(text)
    && !isGenericSceneMarker(text)
    && !isWeakDisplayTitle(text)
    && text.length <= 38
    && countWords(text) <= 7;
}

function goodPlannedTitle(value) {
  const text = stripDisplayTitlePrefix(value);
  const key = normalizeScriptKey(text);
  if (!text || !key || isTechnicalTitle(text) || isGenericSceneMarker(text) || isWeakDisplayTitle(text)) return false;
  if (/^(?:hook|cta|value|proof|evidence|workflow|counter|counterargument|y\s*\d*|ý\s*\d*|canh\s*\d*|cảnh\s*\d*)$/iu.test(key)) return false;
  return text.length <= 38 && countWords(text) <= 7;
}

function isWeakDisplayTitle(value) {
  const text = cleanMarkdownLine(value);
  if (!text) return true;
  if (/^(?:từ đó|nhưng|và|hoặc|nếu|khi|hãy|thực tế|nhìn từ|không phải|ban đầu|đừng|câu hỏi|lý do)\b/iu.test(text)) return true;
  if (/^(?:điều quan trọng|phần đáng chú ý|có bằng chứng|người xem cần|bạn có|bạn muốn|nếu bạn muốn|gom các bước|dùng [^ ]+ như|tín hiệu quan trọng nhất là|khác biệt đáng giá|không phải tình huống nào cũng hợp)\b/iu.test(text)) return true;
  const words = countWords(text);
  if (words <= 2 && /^(?:từ đó|nhưng|vì vậy|thử ngay|kết luận)$/iu.test(text)) return true;
  return false;
}

function isGenericSceneMarker(value) {
  const text = cleanMarkdownLine(value);
  const key = normalizeScriptKey(text);
  if (!text || !key) return true;
  if (/^(?:phan\s+|phần\s+)?\d+$/.test(key)) return true;
  if (/^(?:buoc|bước)\s+\d+$/.test(key)) return true;
  if (/^(?:y|ý|canh|cảnh)\s+\d+$/.test(key)) return true;
  if ([
    "mo dau",
    "mở đầu",
    "thuc te",
...
function titleFromLine(line, fallback = "Điểm đáng chú ý") {
  const smart = smartSceneTitle(line, fallback);
  if (goodDisplayTitle(smart)) return smart;
  const derived = displayHeadlineFromLine(line, fallback);
  if (goodDisplayTitle(derived)) return derived;
  const cleaned = vietnameseText(stripInternalScriptPrefix(line), fallback);
  const split = splitNarrationLead(cleaned, fallback);
  const semantic = sceneSemanticPhrases(
    {
    ...
    []
  ).find(goodDisplayTitle);
  if (semantic) return semantic;
  const first = cleaned.split(/(?<=[.!?])\s+|[,;:]\s+/u).find(Boolean) || cleaned;
  return shortText(stripDisplayMetaPrefix(first) || fallback, 46);
}

function smartSceneTitle(line, fallback = "") {
  const text = stripDisplayMetaPrefix(vietnameseText(stripInternalScriptPrefix(line), fallback));
  const key = normalizeScriptKey(text);
  if (!key) return fallback || "";
  const rules = [
    [/quyen rieng tu|du lieu.*khong roi|ban ghi.*khong roi|models?.*khong roi|mo hinh.*khong roi/, "Dữ liệu ở lại máy"],
    [/luong xu ly giong noi|voice i o|dau vao.*dau ra|input.*output/, "Một luồng giọng nói"],
    [/elevenlabs|wisprflow|dam may.*chia|cloud.*chia|hai nhom cong cu/, "Gom hai nửa giọng nói"],
    [/studio.*giong noi|ma nguon mo|mien phi.*ma nguon|open source/, "Studio giọng nói local"],
    [/nhan ban|clone.*giong|giong tu vai mau/, "Nhân bản giọng nói"],
    [/cuda|mlx|gpu|hieu nang|toc do xu ly/, "Tận dụng GPU local"],
    [/demo|thao tac|ket qua.*tu danh gia|lam duoc gi/, "Demo dễ kiểm tra"],
    [/rut ngan|doi cach lam cu|truoc day|buoc roi rac/, "Rút ngắn bước cũ"],
    [/khac biet|lam xong viec|ro hon/, "Khác biệt thực dụng"],
    [/khong nen dung|loi gioi thieu|tinh tao|noi qua/, "Chỉ dùng khi rõ lợi ích"],
    [/kiem tra nguon|bang chung|minh hoa|tu kiem tra/, "Bằng chứng dễ kiểm tra"],
    [/luu lai|thu|hay dung|du an nho|viec that/, "Thử với việc thật"]
  ];
...
function stripDisplayMetaPrefix(text = "") {
  let output = String(text || "").trim();
  const prefixPatterns = [
    /^mở bằng(?: một tình huống thật)?\s*/iu,
    /^bước tiếp theo(?: là cho thấy)?\s*/iu,
    /^điểm đáng xem(?: của [^,.:;]+)?\s*/iu,
    /^nếu chỉ được hỏi một câu(?: về [^,.:;]+)?[,:\s]*/iu,
    /^từ thông tin công khai(?: của [^,.:;]+)?[,:\s]*/iu,
    /^kết quả người xem cần là\s*/iu,
    /^cách cũ là\s*/iu,
    /^điểm cần tỉnh táo là\s*/iu,
    /^câu chuyện bắt đầu(?: rất quen)?[:\s]*/iu,
    /^điều quan trọng nhất là\s*/iu,
    /^đây là kiểu dự án cần được hiểu thật nhanh[:\s]*/iu,
    /^một demo ngắn về [^,.:;]+ nên đi thẳng vào bốn việc[:\s]*/iu
  ];
  ...
  return output
    .replace(/^[\p{L}\p{N}_/-]{2,24}\s+(?=(?:gom|giúp|cho|biến|chạy|tận|hỗ|giữ|mở|đưa|nhận|tạo|xử|làm|rút)\b)/iu, "")
    .replace(/^[,.;:!?–—\s-]+/u, "")
    .replace(/^[\p{L}\p{N}_/-]{2,40}\s*[-:–]\s*/u, "")
    .trim();
}

function splitNarrationLead(line, fallback = "") {
  const original = vietnameseText(stripInternalScriptPrefix(line), fallback).trim();
  const cleaned = stripDisplayMetaPrefix(original);
  if (!cleaned) return { headline: fallback || "", body: "" };
  const sentenceParts = cleaned.split(/(?<=[.!?])\s+/u).map((item) => item.trim()).filter(Boolean);
  const questionPivot = cleaned.match(/(.+?\?)\s+(.+)/u);
  if (questionPivot) {
    return {
      headline: stripDisplayMetaPrefix(questionPivot[1].replace(/[?]+$/u, "")),
      body: stripDisplayMetaPrefix(questionPivot[2])
    };
  }
  const phrasePivot = cleaned.match(/^(.{8,84}?)(?=\s+(?:Bạn|Nếu|Khi|Có người|Thực tế|Chỉ cần|Với|Điểm|Nhờ|Tất cả)\b)/u);
  if (phrasePivot) {
    const headline = stripDisplayMetaPrefix(phrasePivot[1]);
    const body = stripDisplayMetaPrefix(cleaned.slice(phrasePivot[1].length));
    if (headline && body) return { headline, body };
  }
  const dashParts = cleaned.split(/\s*[–—:]\s*/u).map((item) => item.trim()).filter(Boolean);
  if (dashParts.length > 1 && countWords(dashParts[0]) <= 10) {
    return {
      headline: stripDisplayMetaPrefix(dashParts[0]),
      body: stripDisplayMetaPrefix(dashParts.slice(1).join(". "))
    };
  }
  if (sentenceParts.length > 1 && countWords(sentenceParts[0]) <= 10) {
    return {
      headline: stripDisplayMetaPrefix(sentenceParts[0]),
      body: stripDisplayMetaPrefix(sentenceParts.slice(1).join(" "))
    };
  }
  const clauseParts = cleaned.split(/\s*[,;]\s+/u).map((item) => item.trim()).filter(Boolean);
  if (clauseParts.length > 1 && countWords(clauseParts[0]) <= 8) {
    return {
      headline: stripDisplayMetaPrefix(clauseParts[0]),
      body: stripDisplayMetaPrefix(clauseParts.slice(1).join(". "))
    };
  }
  return { headline: cleaned, body: sentenceParts.slice(1).join(" ") || "" };
}

function displayHeadlineFromLine(line, fallback = "Điểm đáng chú ý") {
  const original = vietnameseText(stripInternalScriptPrefix(line), fallback);
  const cleaned = stripDisplayMetaPrefix(original);
  const split = splitNarrationLead(cleaned, fallback);
  if (/^(?:một demo ngắn về|đây là kiểu dự án cần được hiểu thật nhanh|cách hiểu nhanh nhất về|nếu đang cần bắt đầu nhanh với)\b/iu.test(original)) {
    return shortText(fallback, 46);
  }
  const parts = (split.headline || cleaned)
    .split(/(?<=[.!?])\s+|[,;:]\s+/u)
    .map((item) => item.trim())
    .filter(Boolean);
  const first = parts[0] || cleaned || fallback;
  const words = wordsFromText(first);
  const compact = words.slice(0, Math.min(7, words.length)).join(" ").trim();
  const text = compact || first || fallback;
  return shortText(text ? text.charAt(0).toUpperCase() + text.slice(1) : fallback, 46);
}

function displayBodyForScene(scene, index, job = null) {
  const original = vietnameseText(scene?.body || "");
  const cleaned = stripDisplayMetaPrefix(original);
  const split = splitNarrationLead(cleaned, publicSceneTitle(scene, index, job));
  if (split.body) {
    const candidate = split.body.replace(/^[,.;:!?–—\s-]+/u, "").trim();
    if (countWords(candidate) >= 7) return conciseText(candidate, 30, 220);
  }
  const headline = normalizeScriptKey(publicSceneTitle(scene, index, job));
  const structuredParts = cleaned.split(/\s*[–—:]\s*/u).map((item) => item.trim()).filter(Boolean);
  if (headline && structuredParts.length > 1) {
    const firstPartKey = normalizeScriptKey(structuredParts[0]);
    const titleKey = headline.split(/\s+/).slice(0, 5).join(" ");
    if (firstPartKey && (firstPartKey === titleKey || firstPartKey.startsWith(titleKey) || titleKey.startsWith(firstPartKey))) {
      return conciseText(structuredParts.slice(1).join(". "), 30, 220);
    }
  }
  const body = cleaned.split(/\s+/).filter(Boolean);
  let trimmed = cleaned;
  if (headline) {
    const titleWords = headline.split(/\s+/).filter(Boolean);
    if (titleWords.length) {
      const bodyHead = normalizeScriptKey(body.slice(0, titleWords.length + 2).join(" "));
      if (bodyHead.startsWith(titleWords.join(" "))) {
        trimmed = body.slice(titleWords.length).join(" ").replace(/^[,.;:!?–—\s-]+/u, "").trim();
      }
    }
  }
  let finalText = (trimmed || cleaned || original).replace(/^[,.;:!?–—\s-]+/u, "").trim();
  if (countWords(finalText) < 7 && countWords(cleaned) > countWords(finalText)) finalText = cleaned;
  return conciseText(finalText, 30, 220);
}

function sceneSemanticPhrases(scene, base = []) {
  const candidates = [
    scene?.title || "",
    scene?.subtext || "",
    scene?.body || "",
  ...
  for (const item of candidates) {
    const cleaned = stripDisplayMetaPrefix(item);
    if (!cleaned) continue;
    const directTitle = finalizeDisplayPhrase(cleaned);
    if (goodDisplayTitle(directTitle)) {
      const key = normalizeScriptKey(directTitle);
      if (key && !seen.has(key)) {
        seen.add(key);
        phrases.push(directTitle);
        if (phrases.length >= 4) break;
        continue;
      }
    }
    const smart = smartSceneTitle(cleaned, "");
    if (smart) {
      const key = normalizeScriptKey(smart);
      if (key && !seen.has(key) && !isTechnicalTitle(smart) && !isGenericSceneMarker(smart)) {
        seen.add(key);
        phrases.push(smart);
        if (phrases.length >= 4) break;
        continue;
    ...
    const head = displayHeadlineFromLine(cleaned, "");
    const label = cueFragmentToLabel(head || cleaned);
    const key = normalizeScriptKey(label);
    if (!key || seen.has(key) || isTechnicalTitle(label) || isGenericSceneMarker(label) || isWeakDisplayTitle(label)) continue;
    seen.add(key);
    phrases.push(finalizeDisplayPhrase(label));
    if (phrases.length >= 4) break;
  }
  for (const item of base) {
    const label = finalizeDisplayPhrase(publicChipLabel(item));
    const key = normalizeScriptKey(label);
...
function sceneTitleFor(line, index, plan, job) {
  ...
  if (goodPlannedTitle(planned) && countWords(stripDisplayTitlePrefix(planned)) <= 6 && !/[.!?]/.test(planned)) return stripDisplayTitlePrefix(planned);
  ...
  const derived = displayHeadlineFromLine(line, fallbackTitles[index] || "Điểm đáng chú ý");
  if (goodDisplayTitle(derived)) return derived;
  const labelTitle = displayLabel(plan.labels?.[index], index);
  if (goodDisplayTitle(labelTitle)) return labelTitle;
  return publicComboSceneTitle(job, index) || derived || fallbackTitles[index] || "Điểm đáng chú ý";
}

function publicComboSceneTitle(job, index) {
  return [
      "Điểm đáng chú ý",
      "Lợi ích rõ nhất",
      "Bằng chứng kiểm tra",
...
function buildScenes(job) {
  ...
  return script.map((line, index) => ({
    ...
    label: publicChipLabel(plan.labels?.[index] || sceneSemanticPhrases({ title: "", subtext: "", body: line }, [])[0] || "Điểm chính", index),
...
async function generateVietnameseVoice(job, workspace, onLog) {
  ...
  const narration = narrationLines
    .map((line, index, lines) => splitTextBySentences(normalizeVietnameseSpeechText(vietnameseText(narrationReadyLine(line, index, lines)))).join("\n"))
    .filter(Boolean)
    .join("\n\n");
...
async function resolveJobScript(job, state, onLog) {
  ...
  try {
    ...
    job.contentPlan = {
      ...
      styleLabel: "LLM",
      titles: llm.scenes.map((scene, index) => {
        const candidate = finalizeDisplayPhrase(stripDisplayTitlePrefix(scene.title || ""));
        if (goodPlannedTitle(candidate)) return candidate;
        const derived = finalizeDisplayPhrase(titleFromLine(scene.voice || scene.subtitle || "", "Điểm đáng chú ý"));
        if (goodDisplayTitle(derived)) return derived;
        return derived || "Điểm đáng chú ý";
      }),
      labels: displayLabels(llm.scenes.map((scene, index) => {
        const direct = finalizeDisplayPhrase(scene.label || "");
        if (goodDisplayTitle(direct)) return direct;
        const derived = sceneSemanticPhrases(
          {
            title: scene.title || "",
            subtext: scene.subtitle || "",
            body: scene.voice || ""
          },
          []
        )[0];
        return derived || direct || `Điểm ${index + 1}`;
      }), llm.scenes.length),
...
function syntheticVisualHtml(index, scene, labels, icons, job = null) {
  const title = conciseText(publicSceneTitle(scene, index, job), 6, 34);
  const text = conciseText(displayBodyForScene(scene, index, job) || scene.subtext || scene.body, 12, 84);
  const comboId = job?.options?.templateCombo || "repo-launch-dynamic";
  const iconItems = (labels.length ? labels : icons).slice(0, 6);
  const variantMap = {
    "repo-launch-dynamic": ["burst", "terminal", "flow", "orbit", "cta"],
    "evidence-explainer": ["terminal", "flow", "burst", "orbit", "cta"],
    "infographic-cta": ["orbit", "burst", "flow", "cta", "orbit"],
    "app-demo-flow": ["terminal", "flow", "terminal", "orbit", "cta"],
    "privacy-trust-review": ["orbit", "flow", "terminal", "burst", "cta"],
    "quick-compare": ["flow", "burst", "orbit", "terminal", "cta"],
    "tutorial-steps": ["flow", "terminal", "flow", "cta", "orbit"],
    "creator-story": ["burst", "orbit", "flow", "cta", "terminal"]
  };
  const variant = (variantMap[comboId] || variantMap["repo-launch-dynamic"])[index % 5];
  if (variant === "terminal") {
    return `
...
✓ ${escapeHtml(text)}</code></pre>
  ...
  if (variant === "flow") {
    const flowNodes = comboFlowNodes(comboId, scene);
    return `
      <div class="synthetic flow-visual">
        ...
        ${[title, flowNodes[0], flowNodes[1], flowNodes[2], publicChipLabel("result")].map((node, i) => `<em class="big-node node-${i}">${escapeHtml(conciseText(node, 4, 22))}</em>`).join("")}
      </div>`;
  }
  if (variant === "orbit") {
    return `
...
function cueFragmentToLabel(fragment) {
  const cleaned = stripDisplayMetaPrefix(fragment)
    .replace(/[.!?]+$/u, "")
    .replace(/^(?:và|nhưng|nếu|thì|đó là|đó|hãy|chỉ|có|một|toàn bộ|người xem|đừng|phần|điểm)\s+/iu, "")
    .trim();
  if (!cleaned) return "";
  const words = wordsFromText(cleaned).filter((word) => word.length > 1);
  if (!words.length) return "";
  if (words.length <= 2 && /^(?:nhanh|hơn|và|rồi|đều|đang|được)$/iu.test(words.join(" "))) return "";
  return shortText(words.slice(0, Math.min(3, words.length)).join(" "), 18);
}

function dynamicSceneLabels(scene, base = []) {
  const semantic = sceneSemanticPhrases(scene, []);
  if (semantic.length >= 3) return semantic;
  const candidates = [
    scene?.title || "",
    scene?.subtext || "",
    ...(stripDisplayMetaPrefix(scene?.body || "").split(/(?<=[.!?])\s+|[,;:]\s+/u).map((item) => item.trim()).filter(Boolean))
  ];
  const seen = new Set();
  const labels = [];
  for (const item of candidates) {
    const label = cueFragmentToLabel(item);
    const key = normalizeScriptKey(label);
    if (!key || seen.has(key) || isTechnicalTitle(label)) continue;
    seen.add(key);
    labels.push(label);
    if (labels.length >= 4) break;
  }
  if (labels.length < 4) {
    const bodyWords = wordsFromText(stripDisplayMetaPrefix(scene?.body || ""));
    for (let index = 0; index < bodyWords.length - 1; index += 2) {
      const label = cueFragmentToLabel(bodyWords.slice(index, index + 3).join(" "));
      const key = normalizeScriptKey(label);
      if (!key || seen.has(key) || isTechnicalTitle(label)) continue;
      seen.add(key);
      labels.push(label);
  ...
  for (const item of base) {
    const label = publicChipLabel(item);
    const key = normalizeScriptKey(label);
    if (!key || seen.has(key)) continue;
    seen.add(key);
    labels.push(label);
...
function comboSemanticLabels(comboId, index, scene, job) {
  const base = [scene?.title, scene?.subtext, scene?.body].filter(Boolean);
  return dynamicSceneLabels(scene, base).slice(0, 5);
}

function comboFlowNodes(comboId, scene = null) {
  const base = [scene?.title, scene?.subtext, scene?.body].filter(Boolean);
  const labels = dynamicSceneLabels(scene, base).slice(0, 4);
  const fallback = ["Điểm chính", "Lợi ích", "Kiểm chứng", "Thử tiếp"];
  return labels.length >= 4 ? labels : [...labels, ...fallback].slice(0, 4);
}
...
function publicSceneTitle(scene, index, job = null) {
  const title = String(scene.title || "").trim();
  if (title && goodPlannedTitle(title) && !/[.!?]/.test(title)) {
    return finalizeDisplayPhrase(conciseText(stripDisplayTitlePrefix(title), 7, 46));
  }
  const derived = displayHeadlineFromLine(scene?.body || scene?.title || "", publicComboSceneTitle(job || {}, index));
  if (derived && goodDisplayTitle(derived)) {
    return finalizeDisplayPhrase(conciseText(derived, 7, 46));
  }
  return finalizeDisplayPhrase(conciseText(publicComboSceneTitle(job || {}, index), 7, 46));
}
...
function comboVisualCopy(comboId, index, scene, job, media, fallbackSource = "Nguồn đã chọn") {
  ...
  const body = displayBodyForScene(scene, index, job);
  const rawSubtext = stripDisplayMetaPrefix(scene.subtext || scene.body);
  const subtext = finalizeDisplayPhrase(conciseText(countWords(rawSubtext) >= 6 ? rawSubtext : body || rawSubtext, 10, 64));
  ...
  const captions = {
    ...
    "app-demo-flow": "Cho thấy trải nghiệm chính của sản phẩm",
    "infographic-cta": "Biến ý chính thành tín hiệu trực quan",
    "creator-story": "Kể bằng vấn đề, chuyển biến và kết quả"
  };
  return {
    source,
    title: finalizeDisplayPhrase(conciseText(publicSceneTitle(scene, index, job), 7, 46)),
    proof: body || mediaProof || `${source} được tóm tắt thành ý dễ hiểu cho người xem.`,
    body,
    subtext,
    caption: mediaProof || captions[comboId] || "Thông tin được trình bày trực quan cho người xem"
  };
}

function appDemoActionLines(copy, media) {
  const sourceProof = media
    ? publicMediaProof(media, 0)
    : "Nguồn được rút ý chính để người xem nắm nhanh.";
  const outcome = countWords(copy.subtext) >= 6
    ? copy.subtext
    : countWords(copy.body) >= 6
      ? conciseText(copy.body, 9, 64)
      : "Người xem thấy ngay kết quả chính.";
  return [
    ["Nguồn", conciseText(sourceProof, 9, 68)],
    ["Luồng", conciseText(copy.body || copy.proof, 10, 72)],
    ["Kết quả", conciseText(outcome, 9, 64)],
    ["Chạy trên máy", media ? "Có dẫn chứng trực tiếp từ repo." : conciseText(copy.source, 6, 40)]
  ];
}
...
function chipsHtml(icons, className = "icon-strip") {
  return `<div class="${className}">${icons.slice(0, 4).map((icon) => `<em class="cue">${escapeHtml(conciseText(String(icon || ""), 3, 18))}</em>`).join("")}</div>`;
}
...
function visualHtml(scene, index, job, combo) {
  ...
  const links = Array.isArray(job.links) && job.links.length ? job.links : [job.sourceUrl || job.source || "Hypervideo"];
  const link = links[index % links.length] || "Hypervideo";
  const sourceLabel = (() => {
    try {
      const parsed = new URL(link);
  ...
  const labels = comboSemanticLabels(combo.id, index, scene, job);
  ...
  const useSourceMedia = Boolean(media?.path);
  const preferGeneratedVisual = !useSourceMedia && (
    combo.mediaStrategy === "generated-abstract" || (combo.infographic || "").includes("metric")
  );
  const sceneDuration = Math.max(0.2, Number(scene.end || 0) - Number(scene.start || 0));
  const mediaNode = !preferGeneratedVisual && media?.kind === "video"
    ? `<video id="scene-${index}-source-media" class="source-media" src="${escapeHtml(relMedia)}" muted loop autoplay playsinline data-start="${Math.max(0, Number(scene.start || 0)).toFixed(2)}" data-duration="${sceneDuration.toFixed(2)}"></video>`
    : !preferGeneratedVisual && media?.path
      ? `<img class="source-media" src="${escapeHtml(relMedia)}" alt="source media">`
      : syntheticVisualHtml(index, scene, labels, icons, job);
  const copy = comboVisualCopy(combo.id, index, scene, job, media, sourceLabel);
  const actionPhrases = [
    conciseText(copy.proof, 7, 34),
    conciseText(copy.body, 7, 34),
    conciseText(copy.subtext, 6, 28),
    conciseText(copy.source, 5, 24)
  ].filter(Boolean);
  const words = scene.subtext.split(/\s+/).filter(Boolean).slice(0, 14);
  const showKineticWords = preferGeneratedVisual || ["infographic-cta", "creator-story"].includes(combo.id);
  const kineticWords = showKineticWords
    ? words.map((word, i) => `<span class="kinetic-word cue" style="--i:${i}">${escapeHtml(word)}</span>`).join("")
    : "";
  const flowNodes = comboFlowNodes(combo.id, scene);
  const mediaFrame = mediaFrameHtml(mediaNode, scene, copy);
  const proofCard = proofCardHtml(copy, false);
  const chipSource = combo.id === "app-demo-flow"
    ? (labels.length ? labels : ["Nguồn thật", "Luồng dùng", "Kết quả", "Chạy trên máy"])
    : combo.id === "tutorial-steps"
      ? (labels.length ? labels : ["Chuẩn bị", "Làm thử", "Kiểm tra", "Hoàn tất"])
      : combo.id === "quick-compare"
        ? (labels.length ? labels : ["Trước đó", "Bây giờ", "Khác gì", "Khi nên thử"])
        : ["app-demo-flow", "tutorial-steps", "quick-compare"].includes(combo.id) ? flowNodes : icons;
  const chipStrip = chipsHtml(chipSource);
  const commonTop = `
      <div class="orb one"></div>
      <div class="orb two"></div>
      ${showKineticWords ? `<div class="kinetic-stage cue">${kineticWords}</div>` : ""}`;
  let layout = `
      ${proofCard}
      ${mediaFrame}
  ...
  if (combo.id === "privacy-trust-review") {
    ...
    if (variant === 1) {
      ...
      layout = `
        ...
        <div class="visual-bottom">${chipsHtml(["Chạy trên máy", "Riêng tư", "Kiểm chứng", "Quyết định"])}</div>`;
    } else {
      layout = `
    ...
    const proofStrip = `
        <div class="compare-proof-strip cue">
          <span>${escapeHtml(copy.source)}</span>
          <b>${escapeHtml(copy.title)}</b>
          <small>${escapeHtml(copy.proof)}</small>
        </div>`;
    if (variant === 1) {
      layout = `
        ${mediaFrameHtml(mediaNode, scene, copy, "compare-media compare-spotlight")}
        <div class="compare-timeline cue">
          ${[labels[0] || "Điểm nhìn", labels[1] || "Khác biệt", labels[2] || "Kết quả"].map((label, i) => `<span><b>${escapeHtml(label)}</b><small>${escapeHtml(conciseText([copy.body, copy.subtext, copy.proof][i], 8, 58))}</small></span>`).join("")}
        </div>
        <div class="visual-bottom">${chipStrip}</div>`;
    } else {
      layout = `
        <div class="compare-grid cue">
          <div class="compare-panel before"><span>${escapeHtml(labels[0] || "Trước đó")}</span><p>${escapeHtml(conciseText(copy.body, 10, 74))}</p></div>
          <div class="compare-panel after"><span>${escapeHtml(labels[1] || "Kết quả mới")}</span><p>${escapeHtml(conciseText(copy.subtext || copy.proof, 10, 74))}</p></div>
        </div>
        ${variant === 2 ? proofStrip : mediaFrameHtml(mediaNode, scene, copy, "compare-media")}
        ${variant === 2 ? mediaFrameHtml(mediaNode, scene, copy, "compare-media") : ""}
        <div class="visual-bottom">${chipStrip}</div>`;
    }
  } else if (combo.id === "tutorial-steps") {
    const steps = ["Chuẩn bị", "Chạy thử", "Kiểm tra"];
    const stepCards = `<div class="step-stack cue compact-steps">
      ${steps.map((step, i) => `<div class="step-card"><b>${escapeHtml(step)}</b><span>${escapeHtml(conciseText(i === 0 ? copy.proof : i === 1 ? copy.body : copy.subtext, 7, 36))}</span><p>${escapeHtml(conciseText(i === 0 ? copy.proof : i === 1 ? copy.body : copy.subtext, 9, 66))}</p></div>`).join("")}
    </div>`;
    layout = variant === 1
    ...
    const actionLines = appDemoActionLines(copy, media);
    const actions = `<div class="app-actions cue">
      ${actionLines.map(([label, text]) => `<span><b>${escapeHtml(label)}</b><small>${escapeHtml(text)}</small></span>`).join("")}
    </div>`;
    layout = `
      <div class="app-demo-shell cue">
        ${proofCardHtml({
          ...copy,
          proof: conciseText(copy.proof || copy.body || copy.subtext, 13, 110)
        }, true)}
        ${mediaFrameHtml(mediaNode, scene, copy, "app-media")}
        ${actions}
        <div class="visual-bottom">${chipStrip}</div>
      </div>`;
    ...
    layout = `
      <div class="launch-board cue">
        <div class="launch-copy">
          <span>${escapeHtml(copy.source)}</span>
          <strong>${escapeHtml(publicSceneTitle(scene, index, job))}</strong>
          <p>${escapeHtml(copy.body || copy.proof)}</p>
        </div>
...
function renderHtml(job, catalogs, options = {}) {
  ...
  const sceneHtml = scenes.map((scene, index) => `
    <section id="scene${index}" class="scene scene-${index % 3}">
      <div class="copy">
        <div class="eyebrow title-pill cue">${escapeHtml(publicSceneTitle(scene, index, job))}</div>
        <p class="body-copy cue">${escapeHtml(displayBodyForScene(scene, index, job))}</p>
  ...
  const timelineJs = scenes.map((scene, index) => {
    ...
    tl.to("#scene${index} .visual-card, #scene${index} .trust-radar, #scene${index} .trust-ledger, #scene${index} .trust-card, #scene${index} .compare-proof-strip, #scene${index} .compare-grid, #scene${index} .compare-panel, #scene${index} .compare-timeline, #scene${index} .step-stack, #scene${index} .step-card, #scene${index} .metric-grid, #scene${index} .metric-pill, #scene${index} .app-actions, #scene${index} .app-actions span, #scene${index} .launch-board, #scene${index} .launch-copy, #scene${index} .flow-mini", {autoAlpha:1, y:0, duration:0.3, stagger:0.035, ease:"power2.out"}, ${at(0.2, 0.58)});
    ...
    ${index < scenes.length - 1 ? `tl.fromTo("#grid-pixelate-overlay .grid-cell", {scale:0, autoAlpha:0}, {scale:0.72, autoAlpha:0.7, duration:0.16, stagger:{amount:0.12, from:"center"}, ease:"power2.inOut"}, ${Math.max(scene.end - 0.28, scene.start + 0.5)});
    tl.fromTo("#grid-pixelate-overlay .grid-cell", {scale:0.72, autoAlpha:0.7}, {scale:0, autoAlpha:0, duration:0.16, stagger:{amount:0.12, from:"edges"}, ease:"power2.inOut"}, ${Math.max(scene.end - 0.12, scene.start + 0.7)});` : ""}
  ...
  const compositionAttrs = previewMode
    ? `data-preview="true" data-width="${aspect.width}" data-height="${aspect.height}"`
    : `data-composition-id="main" data-start="0" data-duration="${duration}" data-width="${aspect.width}" data-height="${aspect.height}"`;
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .compare-proof-strip { position:relative; z-index:3; min-height:96px; padding:18px 22px; border-radius:24px; background:rgba(255,255,255,.9); color:#0f172a; border:1px solid ${palette.primary}44; box-shadow:0 16px 42px rgba(0,0,0,.18); display:grid; grid-template-columns:auto 1fr; gap:8px 16px; align-items:center; }
    .compare-proof-strip span { color:${palette.primary}; font-size:17px; font-weight:900; text-transform:uppercase; }
    .compare-proof-strip b { justify-self:end; color:${palette.primary}; font-size:17px; text-transform:uppercase; }
    .compare-proof-strip small { grid-column:1 / -1; color:#334155; font-size:20px; line-height:1.22; }
    ...
    .app-actions { position:relative; z-index:3; display:grid; grid-template-columns:repeat(2,1fr); gap:12px; }
    .app-actions span { min-height:110px; padding:18px; border-radius:22px; background:rgba(255,255,255,.12); border:1px solid ${palette.primary}55; display:grid; align-content:center; gap:8px; text-align:left; }
    .app-actions b { font-size:26px; color:#fff; }
    .app-actions small { font-size:16px; color:#cbd5e1; }
    .app-demo-shell { position:relative; z-index:3; display:grid; grid-template-columns:1fr; grid-template-rows:auto auto auto auto; gap:16px; min-height:0; }
    .app-demo-shell .proof-card { order:1; }
    .app-demo-shell .media-frame { order:2; }
    .app-demo-shell .app-actions { order:3; }
    .app-demo-shell .visual-bottom { order:4; }
    ...
    .object-stage { position:relative; min-height:300px; aspect-ratio:16 / 10; overflow:hidden; border-radius:20px; background:#020617; display:grid; place-items:center; }
    .source-media { display:block; width:100%; height:100%; min-height:100%; object-fit:contain; border-radius:20px; background:#020617; }
    ...
    #grid-pixelate-overlay .grid-cell { background:${palette.bg}; transform:scale(0); opacity:0; transform-origin:center; }
    ...
    #root.combo-app-demo-flow .scene { grid-template-columns:1fr; grid-template-rows:auto minmax(0,1fr); align-items:stretch; gap:20px; padding:84px 58px 80px; }
    #root.combo-app-demo-flow .copy { align-self:auto; max-width:920px; }
    #root.combo-app-demo-flow .title-pill { font-size:36px; }
    #root.combo-app-demo-flow .body-copy { font-size:25px; max-width:920px; }
    #root.combo-app-demo-flow .meta { max-width:920px; }
    #root.combo-app-demo-flow .visual { min-width:0; max-width:920px; width:100%; justify-self:center; }
    ...
    #root.combo-infographic-cta .source-media { max-height:none; object-fit:contain; border-radius:24px; box-shadow:0 24px 56px rgba(0,0,0,.28); }
    #root.combo-app-demo-flow .visual { grid-template-rows:minmax(0,1fr); padding:22px; }
    #root.combo-app-demo-flow .proof-card { border-radius:18px; }
    #root.combo-app-demo-flow .media-frame { grid-template-rows:minmax(360px,1fr) auto auto; border-radius:26px; background:rgba(2,6,23,.42); }
    #root.combo-app-demo-flow .object-stage { min-height:460px; }
    #root.combo-app-demo-flow .source-media { width:100%; height:100%; max-height:none; object-fit:contain; }
    ...
    #root.combo-quick-compare .proof-card, #root.combo-quick-compare .compare-proof-strip, #root.combo-quick-compare .compare-grid, #root.combo-quick-compare .media-frame, #root.combo-quick-compare .visual-bottom { grid-column:1 / -1; }
    ...
    #root.combo-privacy-trust-review .source-media { max-height:none; object-fit:contain; }
    ...
    #root.combo-app-demo-flow .scene { gap:18px; padding:78px 58px 78px; }
    #root.combo-tutorial-steps .scene { gap:18px; padding:78px 58px 78px; }
    ...
    #root.combo-app-demo-flow .app-demo-shell,
    ...
    #root.combo-quick-compare .compare-proof-strip,
    ...
    #root.combo-app-demo-flow .app-media,
    #root.combo-tutorial-steps .tutorial-media {
      max-width:820px;
      justify-self:center;
    }
    #root.combo-app-demo-flow .media-frame { align-self:start; }
    #root.combo-app-demo-flow .object-stage,
    #root.combo-tutorial-steps .object-stage {
      min-height:0;
      aspect-ratio:16 / 10;
      max-height:520px;
    }
    #root.combo-app-demo-flow .source-media,
    #root.combo-tutorial-steps .source-media {
      height:100%;
      min-height:0;
      max-height:520px;
    }
    #root.combo-app-demo-flow .object-caption,
    #root.combo-tutorial-steps .object-caption {
      display:none;
    }
    ` : ""}
...
<body>
  <div id="root" class="combo-${escapeHtml(combo.id)}" ${compositionAttrs}>
    ${audioTag}
  ...
  <script>
    ...
    const tl = gsap.timeline({paused:${previewMode ? "false" : "true"}});
    ${timelineJs}
    tl.to("#bar", {width:"100%", duration:${duration}, ease:"none"}, 0);
    window.__timelines = window.__timelines || {};
    window.__timelines.main = tl;
    ${previewMode ? `
      const previewFrame = Math.min(Math.max(0, tl.duration() - 0.2), Math.max(1.15, tl.duration() * 0.04));
      try {
        tl.seek(previewFrame);
      } catch (error) {}
...
async function processJob(job, state, onLog) {
  ...
  if (job.options.voice && job.options.voice !== "none") {
    ...
    if (job.voicePath && fs.existsSync(job.voicePath)) {
      ...
      if (voiceDuration > 0) {
        const syncedDuration = Math.max(6, Math.ceil((voiceDuration + 1.1) * 10) / 10);
  ...
  const previewHtmlPath = path.join(workspace, "preview.html");
  fs.writeFileSync(previewHtmlPath, renderHtml(job, catalogs, { preview: true }));
  job.previewPath = previewHtmlPath;

  const outPath = path.join(workspace, "output.mp4");
  const cli = state.settings.hyperframesCli;
  if (!cli || !fs.existsSync(cli)) {
    job.result = { previewPath: previewHtmlPath, renderSkipped: "HyperFrames CLI not found" };
    return job.result;
  }
  ...
  if (job.voicePath) {
    ...
    const voiceDuration = await mediaDuration(job.voicePath);
    const finalDuration = voiceDuration > 0 ? Math.ceil((voiceDuration + 0.42) * 1000) / 1000 : 0;
    if (voiceDuration > videoDuration + 0.25) {
      onLog?.(`Rendered video ${videoDuration.toFixed(2)}s is shorter than voice ${voiceDuration.toFixed(2)}s; extending last frame to keep narration complete.`);
    } else if (videoDuration > voiceDuration + 0.35 && voiceDuration > 0) {
      onLog?.(`Rendered video ${videoDuration.toFixed(2)}s is longer than voice ${voiceDuration.toFixed(2)}s; trimming final MP4 to voice.`);
    }
    ...
    if (voiceDuration > videoDuration + 0.05) {
      muxArgs.push(
        "-vf", `tpad=stop_mode=clone:stop_duration=${(voiceDuration - videoDuration + 0.46).toFixed(3)}`,
        "-c:v", "libx264",
        "-pix_fmt", "yuv420p",
        "-preset", "veryfast",
        "-crf", "18"
    ...
    muxArgs.push(
      "-c:a", "aac",
      "-b:a", "192k",
      ...(finalDuration ? ["-t", String(finalDuration)] : []),
      "-movflags", "+faststart",
      finalPath
    );
    await run("ffmpeg", muxArgs, { cwd: workspace, onLog });
